<template>
  <div>
    <div class="chatShop" v-show="isHomeShow">
      <!-- narBar -->
      <narBar class="narBar">
        <div slot="center" style="letter-spacing: 1.5px; color: #f26">
          欧丽雅商家客服
        </div>
      </narBar>
      <!-- 聊天列表 -->
      <ul>
        <li
          ...
          :key="index"
          @click="goChatUser(index)"
        >
          <img :src="item[item.length - 1].img" alt="" />
          <div class="liDiv1">
            <span>{{ item[0].name }}</span
            ><span>{{ chatLastMsg[index] }}</span>
          </div>
          <div class="liDiv2">
            <span>{{ item[item.length - 1].time }}</span>
          </div>
        </li>
      </ul>
    </div>
