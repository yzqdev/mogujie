<template>
  <div class="upload" v-show="isShow">
    ...
    <narBar class="narBar">
      ...
      <div slot="center" style="letter-spacing: 1.5px; font-size: 15px">
        选择图片
      </div>
      <div slot="right">
        <span style="padding: 1px; font-size: 12px" @click="confirmImg"
          >确定</span
        >
      </div>
    </narBar>
  ...
  methods: {
    ...
    drawChoose() {
      const data = this.ctx.getImageData(
        this.chooseImg.x,
        this.chooseImg.y,
        this.chooseImg.width,
        this.chooseImg.height
      );
      this.chooseRgb = data;
    },
    drawRect() {
